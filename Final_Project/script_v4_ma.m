...
feat_SIFT = cell(1,N);
desc_SIFT = cell(1,N);
feat_HARRIS = cell(1,N);
valid_corners_HARRIS = cell(1,N);
feat_SURF = cell(1,N);
valid_points_SURF = cell(1,N);
all_feature_points = cell(1,N);



aff_harris_features = dir('model_castle/*.png.haraff.sift');
aff_hessian_features = dir('model_castle/*.png.hesaff.sift');


n_points_SIFT = zeros(1,N);
n_points_HARRIS = zeros(1,N);
% feature extraction w/ multiple methods
fprintf('extracting features: finished image ');
for i = 1:N
    % SIFT feature extraction
    [feat_SIFT{i}, desc_SIFT{i}] = vl_sift(castle_gray{i}, 'PeakThresh', 0.03, 'EdgeThresh', 20);
    % HARRIS feature extraction
    corners_HARRIS = detectHarrisFeatures(castle_gray{i},'MinQuality',0.02,'FilterSize',3); % default: 0.01, 5
    [feat_HARRIS{i}, valid_corners_HARRIS{i}]  = extractFeatures(castle_gray{i},corners_HARRIS);
    % SURF feature extraction
    points_SURF = detectSURFFeatures(castle_gray{i},'MetricThreshold',500,'NumOctaves',4); %default: 100, 3
    [feat_SURF{i}, valid_points_SURF{i}] = extractFeatures(castle_gray{i},points_SURF);
    
    matrix_harris = dlmread([harris_files(i).folder '/' harris_files(i).name], ' ', 2, 0);
    matrix_hessian = dlmread([hessian_files(i).folder '/' hessian_files(i).name], ' ', 2, 0);
    
    points_HARRIS{i} = 
    % combine all valid points in 2xN format
    all_feature_points{i} = [feat_SIFT{i}(1:2,:) valid_corners_HARRIS{i}.Location' valid_points_SURF{i}.Location'];
    % get point sizes for merging
    n_points_SIFT(i)= size(feat_SIFT{i},2);
    n_points_HARRIS(i) = valid_corners_HARRIS{i}.Count;
    fprintf('%d/%d... ', i,N);    
end
...
imshow(castle{i});
hold on;
plot(valid_corners_HARRIS{i}.Location(:,1)',valid_corners_HARRIS{i}.Location(:,2)','ro','MarkerSize',10);
subplot(2,3,3);
imshow(castle{i});
hold on;
plot(valid_points_SURF{i}.Location(:,1)',valid_points_SURF{i}.Location(:,2)','bo','MarkerSize',8);
i = 2;
subplot(2,3,4);
imshow(castle{i});
hold on;
...
matches_SIFT = cell(1,N);
matches_HARRIS = cell(1,N);
matches_SURF = cell(1,N);
% pre-allocation of matched point locations
matched_points_SIFT_left = cell(1,N);
matched_points_SIFT_right = cell(1,N);
...
for i = 1:N
    if i < N
        ...
        matches_SIFT{i} = vl_ubcmatch(desc_SIFT{i},desc_SIFT{i+1});
        matches_HARRIS{i} = matchFeatures(feat_HARRIS{i}, feat_HARRIS{i+1});
        matches_SURF{i} = matchFeatures(feat_SURF{i}, feat_SURF{i+1});
        % find matched points coordinates in right image
        matched_points_SIFT_right{i} = feat_SIFT{i+1}(1:2,matches_SIFT{i}(2,:));
        matched_points_HARRIS_right{i} = valid_corners_HARRIS{i+1}(matches_HARRIS{i}(:,2));
        matched_points_SURF_right{i} = valid_points_SURF{i+1}(matches_SURF{i}(:,2));
    else
        % find match indices in both images
        matches_SIFT{i} = vl_ubcmatch(desc_SIFT{i},desc_SIFT{1});
        matches_HARRIS{i} = matchFeatures(feat_HARRIS{i}, feat_HARRIS{1});
        matches_SURF{i} = matchFeatures(feat_SURF{i}, feat_SURF{1});
        % find matched points coordinates in right image    
        matched_points_SIFT_right{i} = feat_SIFT{1}(1:2,matches_SIFT{i}(2,:));
        matched_points_HARRIS_right{i} = valid_corners_HARRIS{1}(matches_HARRIS{i}(:,2));
        matched_points_SURF_right{i} = valid_points_SURF{1}(matches_SURF{i}(:,2));
        %pR{i} = feats{1}(1:2,matches{i}(2,:));
    end
    % find matched points coordinates in left image  
    %pL{i} = feats{i}(1:2,matches{i}(1,:));
    matched_points_SIFT_left{i} = feat_SIFT{i}(1:2,matches_SIFT{i}(1,:));
    matched_points_HARRIS_left{i} = valid_corners_HARRIS{i}(matches_HARRIS{i}(:,1));
    matched_points_SURF_left{i} = valid_points_SURF{i}(matches_SURF{i}(:,1));
    fprintf('%d/%d... ', i,N);        
end
fprintf('\nFinished computing matches.\n');


%% Combine match indices and points
...
for i = 1:N
    % combine the matched point indices in 2xN format
    if i < N
        match_indices{i}(1,:) = [matches_SIFT{i}(1,:) matches_HARRIS{i}(:,1)'+n_points_SIFT(i) matches_SURF{i}(:,1)'+n_points_SIFT(i)+n_points_HARRIS(i)];
        match_indices{i}(2,:) = [matches_SIFT{i}(2,:) matches_HARRIS{i}(:,2)'+n_points_SIFT(i+1) matches_SURF{i}(:,2)'+n_points_SIFT(i+1)+n_points_HARRIS(i+1)];
    else
        match_indices{i}(1,:) = [matches_SIFT{i}(1,:) matches_HARRIS{i}(:,1)'+n_points_SIFT(i) matches_SURF{i}(:,1)'+n_points_SIFT(i)+n_points_HARRIS(i)];
        match_indices{i}(2,:) = [matches_SIFT{i}(2,:) matches_HARRIS{i}(:,2)'+n_points_SIFT(1) matches_SURF{i}(:,2)'+n_points_SIFT(1)+n_points_HARRIS(1)];
    end
    % combine matched points in 2xN format
    matched_points_left{i} = [matched_points_SIFT_left{i} matched_points_HARRIS_left{i}.Location' matched_points_SURF_left{i}.Location']; 
    matched_points_right{i} = [matched_points_SIFT_right{i} matched_points_HARRIS_right{i}.Location' matched_points_SURF_right{i}.Location']; 
end
    
%%
j = 1;
figure;
plotMatches(castle{j},castle{j+1},matched_points_left{j}(:,:),matched_points_right{j}(:,:));
...
for i = 1:N
    ...
    [Fund_mat(:,:,i), idx_inliers{i}, best_matched_points_left{i}, best_matched_points_right{i}]...
        = eight_point_ransac(matched_points_left{i}, matched_points_right{i}, 0.0005);
